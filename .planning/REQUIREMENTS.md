...
- [x] **INT-01**: IDL generated by `anchor build` is automatically copied to frontend and oracle so all three subsystems share the same type definitions
- [x] **INT-02**: Environment configuration (RPC URL, program ID, round timing, WebSocket endpoint) is managed via env vars across all three subsystems with a shared .env.example
